...
export default function StoryForm({ onStoryGenerated }: { onStoryGenerated: (pages: Page[]) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <input type="number" className="w-full p-2 border rounded" placeholder="Age" value={age} onChange={e => setAge(Number(e.target.value))} />
      <input className="w-full p-2 border rounded" placeholder="Interests (comma-separated)" value={interests} onChange={e => setInterests(e.target.value)} />
      <select className="w-full p-2 border rounded" value={length} onChange={e => setLength(Number(e.target.value))}>
